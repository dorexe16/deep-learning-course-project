...
train_transform = transforms.Compose([
    ...
    ElasticTransform(alpha=36, sigma=6),# Convert images to grayscale with a probability
    transforms.ToTensor(),
...
dtd_data_val = datasets.DTD(root="./data", split="val", download=True, transform=val_transform)

# To overfit, select a small subset of the dataset
# num_samples = len(dtd_data_train)
...
# Create data loaders
train_loader = DataLoader(dtd_data_train, batch_size=batch_size, shuffle=True, num_workers=4)
val_loader = DataLoader(dtd_data_val, batch_size=batch_size, shuffle=False, num_workers=4)
...
#
# model = ViTForImageClassification.from_pretrained("./overfitted_vit_model",num_labels=47,ignore_mismatched_sizes=True)
# model.to(device)
...
trainer = Trainer(
    model=model,
    args=training_args,
    train_dataset=dtd_data_train,
    eval_dataset=dtd_data_val,
...
# )
# fine_tuned_model.to(device)  # Move model to GPU
...
# fine_tuned_model = ViTForImageClassification.from_pretrained("./overfitted_vit_model",
#                                                              num_labels=9,
#                                                              ignore_mismatched_sizes=True)
...
fine_tuning_args = TrainingArguments(
    ...
    eval_strategy="epoch",
    fp16=False,
    fp16_full_eval=False,  # Enable FP16 evaluation
)
