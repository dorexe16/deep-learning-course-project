...
train_transform = transforms.Compose(
    [
        ...
        transforms.RandomApply(
            [transforms.RandomAffine(degrees=0, translate=(0.1, 0.1))], p=0.1
    ...
    def __init__(self, root_dir, transform=None):
        ...
        for original_label in os.listdir(root_dir):
            ...
            if original_label not in class_mapping:
                continue  # Skip labels not in the class_mapping
...
indices = {
    label: np.where(np.array(train_dataset.labels) == label)[0]
...
for label, idx in indices.items():
    ...
    new_indices.extend(np.random.choice(idx, max_count - len(idx)))

train_loader = DataLoader(
    Subset(train_dataset, new_indices), batch_size=16, shuffle=True
)
...
class CustomTrainer(Trainer):
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        loss = nn.CrossEntropyLoss(
            weight=torch.tensor(list(class_weights.values())).to(logits.device)
        )(logits, labels)
...
fine_tuned_model = ViTForImageClassification.from_pretrained(
    "./overfitted_vit_model", num_labels=3, ignore_mismatched_sizes=True
)
